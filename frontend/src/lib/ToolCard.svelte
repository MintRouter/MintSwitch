<script lang="ts">
  ...
  // (an Anthropic-side limit) — surfaced as an ⓘ toggletip next to the toggle
  // (click to open/close, Esc or outside click dismisses) so it takes no card
  // height and never pops up on mere hover/focus.
  const claudeOnlyAll = $derived(tool.id === "claude-code" || tool.id === "claude-desktop");
  let infoOpen = $state(false);
  let infoWrapEl = $state<HTMLSpanElement | null>(null);
  // Close the toggletip on any click outside its wrapper (the ⓘ button's own
  // click toggles it and is inside the wrapper, so it is unaffected).
  function onWindowClick(e: MouseEvent): void {
    if (infoOpen && infoWrapEl && !infoWrapEl.contains(e.target as Node)) infoOpen = false;
  }
  ...
  // Esc dismisses the mode-info toggletip first (one thing per press — it
  // never also closes the tiers dialog); otherwise it closes the dialog. Tab
  // is trapped inside the dialog while it is open.
  function onTiersKeydown(e: KeyboardEvent): void {
    if (e.key === "Escape" && infoOpen && !e.defaultPrevented) {
      e.preventDefault();
      infoOpen = false;
      return;
    }
...
<svelte:window onkeydown={onTiersKeydown} onclick={onWindowClick} />
...
<style>
  .tool-card{min-width:0;display:flex;flex-direction:column;gap:12px;padding:14px;border:1px solid var(--border);border-radius:15px;background:var(--surface);box-shadow:var(--shadow-card)}.tool-card.is-modified{border-color:color-mix(in srgb,var(--warn) 25%,var(--border))}.tool-card.is-uninstalled{background:color-mix(in srgb,var(--surface) 72%,var(--surface-2))}
