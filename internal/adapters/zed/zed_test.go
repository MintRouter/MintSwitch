...
// strip the MintSwitch provider block and agent default model while
...
func TestRestoreNoBackupStripsManagedProvider(t *testing.T) {
	...
	if err := os.WriteFile(path, []byte(`{"theme":"One Dark"}`), 0o600); err != nil {
		t.Fatal(err)
	}
	if _, err := a.Apply(sampleProfile()); err != nil {
		t.Fatal(err)
	...
	root := readJSON(t, path)
	if _, present := root["language_models"]; present {
		t.Fatalf("language_models must be stripped: %v", root)
	}
	if _, present := root["agent"]; present {
		t.Fatalf("agent must be stripped: %v", root)
	}
	if root["theme"] != "One Dark" {
