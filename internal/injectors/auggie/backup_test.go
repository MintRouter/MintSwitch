...
func TestRemoveRestoresPristineFile(t *testing.T) {
	i, _ := newInjector(t)
	path := i.configPath()
	original := `{"theme":"default-dark","mcpServers":{"local":{"command":"npx"}}}` + "\n"
...
func TestRemoveStripsEntryWhenNoBackup(t *testing.T) {
	i, _ := newInjector(t)
	path := i.configPath()
	existing := `{"theme":"default-dark","mcpServers":{"mintrouter":{"type":"http","url":"` +
		core.DefaultMCPEndpoint + `","headers":{"Authorization":"Bearer k"}},"local":{"command":"npx"}}}`
