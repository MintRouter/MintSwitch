...
func TestIDAndPaths(t *testing.T) {
	...
	if got := i.MCPConfigPaths(); len(got) != 1 || filepath.Base(got[0]) != "settings.json" {
...
func TestDetect(t *testing.T) {
	...
	i.lookPath = func(string) (string, error) { return "/usr/local/bin/auggie", nil }
	if !i.Detect() {
...
func TestInjectPreservesExistingKeys(t *testing.T) {
	i, _ := newInjector(t)
	path := i.configPath()
	writeExisting(t, path, `{"theme":"default-dark","shell":"zsh","mcpServers":{"other":{"type":"http","url":"https://x"}}}`)
	if _, err := i.InjectMCP(spec()); err != nil {
		t.Fatalf("inject: %v", err)
	}
	m := readJSON(t, path)
	if m["theme"] != "default-dark" || m["shell"] != "zsh" {
		t.Fatalf("unrelated top-level keys lost: %+v", m)
	}
