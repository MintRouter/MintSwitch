...
func TestRemoveStripsEntryWhenNoBackup(t *testing.T) {
	i, _ := newInjector(t)
	path := i.configPath()
	existing := `{"theme":"x","mcp":{"mintrouter":{"type":"remote","enabled":true,"url":"` +
		core.DefaultMCPEndpoint + `","headers":{"Authorization":"Bearer k"}},"other":{"type":"remote","url":"https://y"}}}`
...
// mcp object entirely while keeping other keys.
func TestRemoveDropsEmptyServersKey(t *testing.T) {
	...
	m := readJSON(t, path)
	if _, ok := m["mcp"]; ok {
		t.Fatalf("empty mcp should be dropped: %+v", m)
	}
	if m["theme"] != "x" {
		t.Fatalf("top-level key lost: %+v", m)
	}
