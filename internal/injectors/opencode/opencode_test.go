...
func TestIDAndPaths(t *testing.T) {
	...
	if got := i.MCPConfigPaths(); len(got) != 1 || filepath.Base(got[0]) != "opencode.json" {
...
func TestDetect(t *testing.T) {
	...
	i.lookPath = func(string) (string, error) { return "/usr/local/bin/opencode", nil }
	if !i.Detect() {
...
func TestInjectPreservesExistingKeys(t *testing.T) {
	...
	existing := `{"model":"x/y","provider":{"mintrouter":{"npm":"pkg"}},"mcp":{"other":{"type":"remote","url":"https://x"}}}`
