...
var errMissingKey = errors.New("droid: a MintRouter API key is required to inject the MCP server")

// serverEntry builds the JSON object written under mcpServers.<name> for spec:
// an HTTP-transport remote server with a Bearer token in headers.Authorization.
