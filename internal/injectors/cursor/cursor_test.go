...
func TestDetect(t *testing.T) {
	...
	i.lookPath = func(string) (string, error) { return "/usr/local/bin/cursor", nil }
	if !i.Detect() {
...
func TestInjectWritesEntryAnd0600(t *testing.T) {
	...
	if _, hasType := e["type"]; hasType {
		t.Fatalf("cursor entry must not carry a type field: %+v", e)
	}
...
func TestInjectPreservesExistingKeys(t *testing.T) {
	...
	existing := `{"other":"keep","mcpServers":{"local":{"command":"npx","args":["x"]}}}`
